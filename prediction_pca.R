...
if (PCA_YN == 1) {
    cat(sprintf("Running PCA on %d by %d data... ", nrow(datasetX), ncol(datasetX)))
...
out_filename = sprintf("%dclass_dwt%d_grp%d_fold%d", nclasses, DWT_or_raw, ungrouped_or_grouped_lasso, fold_id)
if(nclasses==2) {
...
#    fdr_list = c(0.01, 0.02, seq(0.05, 0.5, 0.05))
    ...
    for(fdr_thresh in fdr_list) {
        in.file = sprintf("selected_features/%s_knockoffs_%s.txt", out_filename, fdr_thresh)
        df = read_delim(in.file, delim=" ", col_types=cols())
        selected_features = 1:nrow(df)
    ...
    out.file <- sprintf("test_errors/%s_lasso_pca.txt", out_filename)
    df.1 %>% write_delim(out.file, delim=" ")
} else {
    df.1 = tibble()
}
